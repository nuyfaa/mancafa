...
var req = args.join(' ')            
		...
		case 'setthumb':
				...
				client.sendMessage(from, 'Suksek owner', text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./me.jpg`)},"productId":"3937202479680283","title":"MANCA BOT","currencyCode":"IDR","priceAmount1000":"999999999000","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}})
				break
	    case 'setthumbb':
				...
				delb = await client.downloadMediaMessage(boij)
				fs.writeFileSync(`me.jpg`, delb)
				client.sendMessage(from, 'Suksek wakil owner', text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync(`./me.jpg`)},"productId":"3937202479680283","title":"MANCA BOT","currencyCode":"IDR","priceAmount1000":"999999999000","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}})
				break
		case 'tovid':
       ...
       if (!isRegistered) return reply(ind.noregis())
		...
		case 'menu':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					try {
					ppnuy = await client.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
					} catch {
					ppnuy = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
					}
                    nuyca = `╭─「 *MANCA BOT* 」
...
${fxsx} NuyFaa
╰──────────` 
manca = await getBuffer(ppnuy)
                    client.sendMessage(from, manca, image, {quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [num]}, displayname: mem, caption: nuyca })
					break
                  case 'addstic':
                case 'adds':
				    if (!isRegistered) return reply( ind.noregis())
		...
		case 'donate':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					client.sendMessage(from, donasi(), text, {quoted: mek2, contextInfo: { forwardingScore: 508, isForwarded: true}})
					break
		case 'menupicture':
		case 'menupict':
		case 'pictmenu':
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata(namo, ator)} ${ran} -o ${ran}`, async (error) => {
									//if (error) {
											...
											// fs.unlinkSync(ran)
											// }
									client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek2, sendEphemeral: true, thumbnail: fs.readFileSync(`./me.jpg`, 'base64')})
									fs.unlinkSync(media)	
									fs.unlinkSync(ran)	
								})
							})
		...
		case 'setprefix':
					...
					if (!isOwner) return reply(ind.ownerb())
					prefix = args[0]
					client.sendMessage(from, `*Prefix berhasil di ubah menjadi* : ${prefix}`, text, {quoted: mek2, sendEphemeral: true, 'base64')
					break 
		case 'setprefixx':
					if (args.length < 1) return
					if (!isAdmin) return reply('*Only Admin bot*')
		...
		case 'demote':
           	...
           	case 'setname':
                ...
                client.sendMessage(from, 'Succes, Ganti Nama Grup', text, {quoted: mek2})
					break
                case 'setdesc':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
                ...
                client.sendMessage(from, 'Succes, Ganti Deskripsi Grup', text, {quoted: mek2})
					break
		case 'listadmin':
					if (!isGroup) return reply(ind.groupo())

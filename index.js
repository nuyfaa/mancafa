...
var req = args.join(' ')            
		...
		case 'setthumb':
				...
				amjay = `Sukses Owner`
				client.sendMessage(from, amjay, text, {quoted: mek2}
				break
	    case 'setthumbb':
				...
				delb = await client.downloadMediaMessage(boij)
				fs.writeFileSync(`me.jpg`, delb)
				man = `Sukses Wakil Owner`
				client.sendMessage(from, man, text, {quoted: mek2}
				break
		case 'tovid':
...
manca = await getBuffer(ppnuy)
		...
		case 'setprefix':
					...
					if (!isOwner) return reply(ind.ownerb())
					prefix = args[0]
					caa = `*Prefix berhasil di ubah menjadi* : ${prefix}`
					client.sendMessage(from, caa, text, {quoted: mek2, sendEphemeral: true, 'base64')
					break 
		case 'setprefixx':
					if (args.length < 1) return
					if (!isAdmin) return reply('*Only Admin bot*')
